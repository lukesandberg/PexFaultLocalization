...
{
    public class Version18: FaultyVersion, IReplace
    {
        public int[] FaultLines { get { return new int[] { 367 }; } }
                ...
                else
                {
                    switch (pat[j])
                    {
                        ...
                        case CCL:
                            ...
                            break;
                        case NCCL:
                            if (/*(lin[i] != NEWLINE) &&*/ (!locate(lin[i], pat, j + 1))) //FAULT: removed first condition
                                advance = 1;
                            break;
                        default:
                            Caseerror(pat[j]);
